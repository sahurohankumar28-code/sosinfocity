...
if (history.scrollRestoration) {
    ...
    function initLightboxMediaViewer() {
        ...
        if (!cards.length || !lightbox || !lightboxImg || !lightboxCap || !closeBtn) return;

        // Open Lightbox Handling Linkage
        cards.forEach(card => {
            card.addEventListener('click', () => {
                const imgSource = card.querySelector('.gallery-img').getAttribute('src');
        ...
        const deactivateLightbox = () => {
            ...
            setTimeout(() => { lightboxImg.setAttribute('src', ''); }, 300); // Purge asset pipeline buffer
        };
    ...
    const bootGallerySystems = () => {
        ...
        console.log("SOS Infocity Gallery Module: Operational Node Architecture Active.");
    };
